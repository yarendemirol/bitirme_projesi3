"""
Modified training script that incorporates neighbourhood demographic features
into the real estate price prediction pipeline.  The original pipeline
provided by the user already performs extensive feature engineering
including parsing of numeric fields, generation of keyword flags from
the listing title and description, leakage‑free computation of price
per square metre (``Fiyat_m2``) and target encoding for categorical
variables.  This version extends the feature set with a handful of
additional columns representing demographic information for the
neighbourhood (``mahalle``) in which each listing resides.  These
features are drawn from the provided merged CSV files and consist of

    * ``Demo_Toplam_Nufus`` – total population (thousands) for the
      neighbourhood, stored as a string like ``"3.988 - Normal"``.
      The numeric portion before the dash is extracted and converted
      to a float.
    * ``Demo_Egitim_Durumu`` – percentage of residents with at least
      university education, e.g. ``"%32 Üniversite ve Üstü"``.  The
      leading percentage is extracted as a float representing the
      share of highly educated residents.
    * ``Demo_Ortalama_Yas`` – average age of residents.  Parsed to a
      float.
    * ``Demo_Medeni_Hal`` – percentage of married residents, e.g.
      ``"%64 Evli"``.  The leading percentage is extracted and
      converted to a float.
    * ``Demo_Sosyo_Ekonomik_Statu`` – categorical socio‑economic
      status indicator (A, B, C, etc.).  This is kept as a
      categorical feature and one‑hot encoded during preprocessing.

Only these additional demographic features are introduced; no other
columns from the merged CSVs are used.  Care has been taken to
prevent target leakage by ensuring that all new features reflect
static demographic properties rather than statistics computed on the
target variable.  The rest of the pipeline mirrors the structure of
the original script:

1. Load and merge the city‑level CSV files.
2. Clean and parse raw columns to numeric or categorical types.
3. Generate keyword and amenity flags.
4. Create leakage‑free price per square metre and target encoded
   features.
5. Train an XGBoost model and a CatBoost model on log‑transformed
   prices, selecting hyperparameters via a modest random search.
6. Compute an optimal ensemble weight between the two models to
   minimise MAE on a validation split.
7. Evaluate on a held‑out test set stratified by city and report
   performance metrics.
8. Save wrapped models for inference.
...
# Whether to merge individual city CSVs into a single file.  Not used
# in this modified script, but retained for completeness.
MERGE_CSV = False

# Directory containing the CSVs.  By default this is the script's
# directory.  If you wish to use a different location, modify
# ``DATA_DIR`` accordingly.
DATA_DIR = Path(".")

# Column name used to deduplicate listings when merging.
DEDUP_COL = "url"

# Canonical name for the raw listing features column.  The cleaned
# ``Ilan_Ozellikleri`` column will be used to generate amenity flags.
ILAN_OZ_COL = "Ilan_Ozellikleri"

# Raw columns to keep from the CSV files.  These columns will be
# loaded prior to parsing.  Additional columns may be present in the
# merged files but will not be retained unless explicitly listed here.
#
# The demographic columns (Demo_*) are included here so they are
...
RAW_KEEP_COLS = [
    ...
    # ilan özellikleri olası isimler
    "Ilan_Ozellikleri", "İlan Özellikleri", "Ilan Ozellikleri", "İlan_Ozellikleri", "Ilan Özellikleri",
    # demographic features from the merged dataset
    "Demo_Toplam_Nufus", "Demo_Egitim_Durumu", "Demo_Ortalama_Yas",
    "Demo_Medeni_Hal", "Demo_Sosyo_Ekonomik_Statu",
]

# Categorical columns to be target encoded.  Using hierarchical target
# encoding on these columns helps capture geographical price patterns
# without leaking information across splits.  The list remains
...
def read_csv_robust(fp: Path) -> pd.DataFrame:
    """Attempt to read a CSV file using a variety of encodings.

    Some of the provided CSVs use UTF‑8 with BOM, CP1254 or Latin‑1
    encodings.  This helper will try several encodings and fall back
    on the Python engine with ``on_bad_lines='skip'`` if necessary.
    """
...
def _clean_and_dedupe_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Strip whitespace from column names and remove duplicate names."""
    df = df.copy()
...
def _keep_only_needed_raw_cols(df: pd.DataFrame) -> pd.DataFrame:
    """Keep only the columns listed in RAW_KEEP_COLS."""
...
def _find_ilan_oz_col(df: pd.DataFrame):
    """Return the first matching Ilan_Ozellikleri column name."""
    candidates = [
        "Ilan_Ozellikleri", "İlan Özellikleri", "Ilan Ozellikleri", "İlan_Ozellikleri", "Ilan Özellikleri"
    ]
    for c in candidates:
...
def merge_city_csv(pattern: str, output: str):
    """
    Merge multiple city CSV files matching a pattern into a single file.
    Duplicate listings based on ``DEDUP_COL`` are removed.  This helper is
    retained for completeness but is not invoked in this script.
    """
...
# =========================================================
# DEMOGRAPHIC FEATURE PARSERS
# =========================================================
def parse_demo_numeric(val: str) -> float:
    """
    Extract the first numeric value from a demographic string.  For
    example, ``"3.988 - Normal"`` becomes ``3.988`` and ``"%64 Evli"``
    becomes ``64``.  If no digits are found the function returns NaN.
    """
    if pd.isna(val):
        return np.nan
    s = str(val)
    # replace comma with dot for decimals and remove percentage signs
    s = s.replace("%", "").replace(",", ".")
...
def load_all_cities():
    """
    Load and concatenate the merged CSV files for İstanbul, Ankara and İzmir.
    The function assumes that the files are located in the ``BASE_DIR`` and
    are named ``istanbul_merged_all.csv``, ``ankara_merged_all.csv`` and
    ``izmir_merged_all.csv``.  Each DataFrame is trimmed to the columns
    listed in ``RAW_KEEP_COLS`` and augmented with an ``Il`` column
    indicating the city.  Duplicate column names are removed.
    """
    files = [
...
def build_X_y(df: pd.DataFrame):
    """
    Parse the raw DataFrame into features ``X`` and target ``y`` (in TL).

    This function performs the following steps:

    1. Identify and clean the ``Ilan_Ozellikleri`` column.
    2. Parse the price into a numeric ``Fiyat_TL`` column.
    3. Parse the location string into separate ``Ilce`` and ``Mahalle`` if
       these are missing.
    4. Parse basic numeric attributes (metre squares, room counts, etc.).
    5. Generate binary flags from ``Krediye Uygunluk`` and ``Site İçerisinde``.
    6. Parse ordinal floor information.
    7. Normalize selected object columns to reduce cardinality noise.
    8. Create keyword flags from the title and description.
    9. Filter outliers and invalid rows based on price and size.
   10. Drop unused raw columns.
   11. Parse demographic features into numeric and categorical columns.

    The returned feature matrix ``X`` does not include the target price
    column.  All numeric fields are coerced to numeric types where
    appropriate.
    """
    df = df.copy()
    df = _clean_and_dedupe_columns(df)
    ...
    # demographic feature parsing
    # Extract numeric values from demographic strings; socio‑economic status is kept as category
    ...
    # socio economic status remains as is; ensure type string for one‑hot
    ...
    drop_useless = [
        ...
        "Bulunduğu Kat",
        # drop original demographic strings after parsing to numeric/categorical
        "Demo_Toplam_Nufus", "Demo_Egitim_Durumu", "Demo_Ortalama_Yas", "Demo_Medeni_Hal",
        # We intentionally retain Demo_Sosyo_Ekonomik_Statu as a categorical feature
    ]
...
def tune_xgboost(X_train, y_train_log, preprocessor):
    ...
    # A modest parameter grid for demonstration.  Feel free to expand this
    # grid or increase n_iter to explore more combinations.
    param_distributions = {
...
def find_best_weights_3(y_val_tl, pred_xgb_tl, pred_cat_tl, pred_lgb_tl, step=0.05):
    """Grid search over 3 model weights (w_xgb + w_cat + w_lgb = 1)."""
    best_w = (1/3, 1/3, 1/3)
    best_mae = float("inf")
...
def train_random_forest(X_tr: pd.DataFrame, y_tr_log: pd.Series,
                       ...
                       preprocessor):
    """Train a Random Forest model on preprocessed features."""
...
def remove_outliers(X: pd.DataFrame, y_tl: pd.Series, max_price=55_000_000, max_net_m2=1000):
    """Remove overpriced and oversized area listings with fixed thresholds (no leakage)."""
    n_before = len(X)
...
def train_lightgbm(X_tr: pd.DataFrame, y_tr_log: pd.Series,
                   X_va: pd.DataFrame, y_va_log: pd.Series,
                   preprocessor):
    """Train a LightGBM model on preprocessed features with early stopping."""
...
class EnsembleWithFeatures:
    ...
    def _get_base_preds_log(self, Xf):
        """Get log-space predictions from all 4 base models."""
