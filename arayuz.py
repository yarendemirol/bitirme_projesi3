...
DISTRICTS_BY_CITY = {
    ...
    "Ankara": ["Çankaya" ,"Keçiören" ,"Yenimahalle" ,"Mamak" ,"Etimesgut" ,"Sincan" ,"Altındağ" ,"Gölbaşı" ,"Pursaklar"
               ,"Polatlı"],
    "İzmir":  ["Konak" ,"Karşıyaka" ,"Bornova" ,"Buca" ,"Bayraklı" ,"Balçova" ,"Narlıdere" ,"Gaziemir" ,"Çiğli"
              ,"Menemen"]
}
...
# ================== ✅ İLÇE LİSTESİ (CSV'den Tüm İlçeler) ==================
...
def build_input_row(
        ...
        tapu_durumu,
        bulundugu_kat, isitma_tipi, kullanim_durumu,
        site_icinde, takas,
    ...
    kull_val, kull_known = canonicalize(kullanim_durumu, KULLAN_CANON, cutoff=0.82, allow_none=False)
    kat_val, kat_known = canonicalize(bulundugu_kat, KAT_CANON, cutoff=0.82, allow_none=False)
    takas_val, takas_known = canonicalize(takas, TAKAS_CANON, cutoff=0.82, allow_none=False)
    ...
    if COL_KULLAN in row:
        row[COL_KULLAN] = kull_val
    if COL_TAPU in row:
        row[COL_TAPU] = tapu_val
    if COL_TAKAS in row:
        row[COL_TAKAS] = takas_val
    if COL_TIPI in row:
    ...
    dbg = {
        ...
        "col_kullanim": COL_KULLAN, "col_isitma": COL_ISITMA, "col_tapu": COL_TAPU,
...
# ================== ŞEHİR SEÇİMİ (GÖRSELLİ BUTON) ==================
CITY_IMAGES = {
    "İstanbul": BASE_DIR / "assets" / "istanbul.jpg",
    "Ankara": BASE_DIR / "assets" / "ankara.jpg",
    "İzmir": BASE_DIR / "assets" / "izmir.jpg",
}
...
sehir = st.selectbox(
    ...
    index=CITY_OPTIONS.index(st.session_state.sehir),
    key="sehir_selectbox"
)
...
# ✅ DÜZELEN KISIM: İlçe seçenekleri artık CSV'den "tüm ilçeler"
...
kullanim_list = safe_unique(df_all, "Kullanım Durumu", safe_unique(df_all, "Kullanim Durumu", ["Boş", "Mal Sahibi Oturuyor", "Kiracı Oturuyor"]))
tapu_list = safe_unique(df_all, "Tapu Durumu", ["Kat Mülkiyeti", "Kat İrtifakı"])
takas_list = safe_unique(df_all, "Takas", ["Yok", "Var"])
...
with col2:
    ...
    kullanim_durumu = st.selectbox("Kullanım Durumu", options=kullanim_list, index=0)

    site_icinde = st.selectbox("Site İçerisinde mi?", options=["Evet", "Hayır"], index=0)
    takas = st.selectbox("Takas", options=takas_list, index=0)
    ...
    try:
        ...
        st.markdown(
            ...
            <div style="
                ...
                margin-top: 10px;
            ">
                <div style="font-size:18px; color:#4a90e2; font-weight:600;">
                    💰 Tahmini Satış Fiyatı
                </div>
                <div style="font-size:34px; font-weight:800; color:#0b2c5f; margin-top:10px;">
                    {tahmini_fiyat:,.0f} TL
                </div>
...
# python -m streamlit run arayuz3.py
